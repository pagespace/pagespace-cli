...
module.exports  = {
    run: function(opts) {
        ...
        requestAsync({
            ...
            var okPages = pages.filter(function(page) {
                return page.status === 200;
            });
            redirectMap = pages.reduce(function(redirectMap, page) {
                if((page.status === 301 || page.status === 302) && page.redirect) {
                    redirectMap[page.url] = page.redirect.url;
                }
                return redirectMap;
            }, {});

            log.info('Found %s ok pages', okPages.length);
            return pages;
        }).map(function(page) {
            ...
            return self._processPage(page, outputDir, redirectMap, log);
    ...
    _processPage: function(page, outputDir, redirectMap, log) {
        ...
        function mapNode(node, pageUrl) {
            ...
            if(isLocal(attr)) {
                ...
                if (tag === 'a') {
                    ...
                    newAttr = convert(pageUrl, lookupRedirect(attr));
                    if(newAttr === '' || /\.\.\/$/.test(newAttr)) {
                        newAttr += 'index';
                    }
                    if (!/\.html$/.test(attr)) {
                        newAttr += '.html';
    ...
    _fetchResources: function(outputDir, host, resources, log) {
        ...
        return Promise.map(resources, function(resource) {
            return new Promise(function(resolve, reject) {
                var resourceFilePath = path.join(outputDir, resource).replace('?', '-');
